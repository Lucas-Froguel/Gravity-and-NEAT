...
class Gravity:
    def __init__(self, planets, center_x=0, center_y=0, ships=None):
    ...
    def position_ship(self, dt=1/60):
        for ship in self.ships:
            ship.control_thrust(self.center_x, self.center_y)
    ...
    def create_ship(self, dt=1/60):
        pos = get_random_number(2) * np.array([self.center_x, self.center_y])
        ship = Spaceship(position=pos, initial_velocity=10*get_random_number(2))
        ship.calculate_initial_position(dt)
